import { query, run } from '@/data/database';
...
export async function getBudgetMonth(
  ...
  const allocations = await getAllocationsByMonth(row.id);
...
export async function createYearWithAllocations(
  ...
  const budgetYear = await createBudgetYear({ monthlyIncome, year, currency });

  const months: BudgetMonth[] = [];

  for (let month = 1; month <= 12; month++) {
    const budgetMonth = await createBudgetMonth({
